...
@Suite("PointingReport")
struct PointingReportTests {

    @Test("Default report is 8 zero bytes")
    func defaultBytes() {
    ...
    @Test("Buttons serialize as little-endian UInt32")
    func buttonsLittleEndian() {
        var report = PointingReport()
        report.buttons = 0xAABBCCDD
        let bytes = report.toBytes()
        #expect(bytes[0] == 0xDD)
        #expect(bytes[1] == 0xCC)
        #expect(bytes[2] == 0xBB)
    ...
    @Test("Signed x/y movement encoded as Int8")
    func signedMovement() {
        var report = PointingReport()
        report.x = -5
        report.y = 10
        let bytes = report.toBytes()
        #expect(bytes[4] == UInt8(bitPattern: Int8(-5)))
        #expect(bytes[5] == 10)
    }

    @Test("Wheel bytes encode correctly")
    func wheelBytes() {
        var report = PointingReport()
        report.verticalWheel = -1
        report.horizontalWheel = 3
        let bytes = report.toBytes()
        #expect(bytes[6] == UInt8(bitPattern: Int8(-1)))
        #expect(bytes[7] == 3)
    }

    @Test("Max/min Int8 boundary values")
    func extremeValues() {
        var report = PointingReport()
        report.x = Int8.max  // 127
        report.y = Int8.min  // -128
        let bytes = report.toBytes()
        #expect(bytes[4] == 127)
        #expect(bytes[5] == UInt8(bitPattern: Int8.min))
    }
}
